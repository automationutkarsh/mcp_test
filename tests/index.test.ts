...
import { APIPromise } from 'mcp-test/core/api-promise';

import util from 'node:util';
import McpTest from 'mcp-test';
import { APIUserAbortError } from 'mcp-test';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['MCP_TEST_LOG'] = undefined;
    });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['MCP_TEST_LOG'] = 'debug';
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['MCP_TEST_LOG'] = 'not a log level';
      const client = new McpTest({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'MCP_TEST_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['MCP_TEST_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['MCP_TEST_BASE_URL'] = 'https://example.com/from_env';
      const client = new McpTest({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['MCP_TEST_BASE_URL'] = ''; // empty
      const client = new McpTest({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('utkarshthesingh1.aaedev.supremomono.com');
    });

    test('blank env variable', () => {
      process.env['MCP_TEST_BASE_URL'] = '  '; // blank
      const client = new McpTest({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('utkarshthesingh1.aaedev.supremomono.com');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['MCP_TEST_BASE_URL'] = 'http://localhost:5000/env';
      const client = new McpTest({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['MCP_TEST_API_KEY'] = 'My API Key';
    const client = new McpTest();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['MCP_TEST_API_KEY'] = 'another My API Key';
    const client = new McpTest({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
